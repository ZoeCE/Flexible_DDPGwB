...
NOCABLE_CKPT="saves/descent_ablation_trueobs_no_cable_latent_pretrain_l8best_20260702_parallel_l8best/ckpt_best_l8.pt"
FULL16_CKPT="saves/descent_ppo_paper_trueobs_residual_pretrain_trainable16_seed270627_20260703/ckpt_best_l8.pt"

mkdir -p "$OUT/plots_nocable" "$OUT/plots_full16"
...
{
  echo "=== Wind-bin L8-best test started at $(date -Is) ==="
  echo "NoCable ckpt: $NOCABLE_CKPT"
  echo "Full16 ckpt:  $FULL16_CKPT"
  echo "episodes_per_bin=256, seed=280704"
  ...
  "$PY" -u test_phase.py \
    ...
    --ckpt "$NOCABLE_CKPT" \
    --disable-cable-obs \
    --benchmark-out "$OUT/nocable_l8best_wind_bins.csv" \
    --plot-out-dir "$OUT/plots_nocable"
  ...
  echo "=== Wind-bin L8-best test finished at $(date -Is) ==="
