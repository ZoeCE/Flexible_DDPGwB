...
tail -n 100 "$OUT/wind_bins_l8best.stdout.log" 2>/dev/null || true
...
for r in rows:
    print(
        f"{r['model']:>16s} wind={r['wind_bin_mps']:>5s} "
        ...
        f"avgAngle={r['avg_swing_angle_deg']} p95Angle={r['p95_swing_angle_deg']} "
