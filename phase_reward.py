...
def compute_descent_reward(env, obs, config, rstate, tracker=None, rl_action=None):
    """
    Descent 段奖励 v11.3 — PID base + RL residual 专用.

    [v11.3 改动]:
      ❌ 删除 step_penalty (-0.02 → 0): 防止 critic 学常数 → dead critic
      ✅ 新增 action_magnitude_penalty: 强制 residual 接近 0 (Olesen 2026)
      ✅ 新增 action_smoothness_penalty: 防 RL 抖动破坏 PID (Mysore 2021)
      ✅ 保留: xy_align, z_descent, swing_ke, tilt, yaw, precision_bonus, success_bonus
    """
    ...
    # ── 稳定性惩罚 (有界) ────────────────────────────────────────────────────
    # [v12 强化] swing_ke_coef 0.8 → 1.5 (用户要求强化防摆)
    ke = _get_swing_ke(env, obs)
    ke_max = float(rcfg.get("swing_ke_max", 0.25))
    r_swing_ke = -float(rcfg.get("swing_ke_coef", 1.5)) * min(ke, ke_max)
    r_tilt = -float(rcfg.get("tilt_coef", 0.4)) * min(tilt, 0.3)
    r_yaw  = -float(rcfg.get("yaw_coef",  0.5)) * min(abs_yaw, 0.3)
    reward += r_swing_ke + r_tilt + r_yaw
    if tracker:
        tracker.add("swing_ke_penalty", r_swing_ke)
    ...
    # ── [v12.3] 绳索动能惩罚 (用户要求, 三段统一)
    # descent 段尤其重要: 接近钢筋时 cable 高频振动会让 payload 失去精度
    cable_ke = _get_cable_kinetic_energy(env)
    ce_thresh = float(rcfg.get("cable_ke_thresh",      0.05))
    ce_coef   = float(rcfg.get("cable_ke_penalty_coef", 0.8))
    ce_max    = float(rcfg.get("cable_ke_penalty_max",  0.06))
    r_cable_ke = 0.0
    if cable_ke > ce_thresh:
        r_cable_ke = -min(ce_coef * (cable_ke - ce_thresh), ce_max)
        reward += r_cable_ke
    ...
    if rstate.prev_dtf is not None:
        delta_dtf = np.clip(rstate.prev_dtf - dtf, -0.010, 0.010)
        coef_mult = 0.5 if z_not_reached else 1.0
        r_xy = delta_dtf * float(rcfg.get("xy_align_coef", 4.0)) * coef_mult
        reward += r_xy
    rstate.prev_dtf = dtf
    if tracker: tracker.add("xy_align_reward", r_xy)
    ...
    rstate.prev_z = payload_z
    if tracker: tracker.add("z_descent_reward", r_z)

    # ── step penalty ─────────────────────────────────────────────────────────
    # [v11.3] step_penalty 默认从 -0.02 → 0, 防止 critic 学常数 → dead critic
    r_step = float(rcfg.get("step_penalty", 0.0))
    if r_step != 0.0:
        reward += r_step
        if tracker: tracker.add("step_penalty", r_step)

    # ── [v11.3 新增] action_magnitude_penalty (Olesen 2026) ──────────────────
    # 强制 residual 接近 0, 仅必要时介入. rl_action ∈ [-1, 1]³
    r_act_mag = 0.0
    if rl_action is not None:
        a_norm_sq = float(np.mean(np.square(np.asarray(rl_action, np.float32))))
        k_act_mag = float(rcfg.get("action_magnitude_coef", 0.02))
    ...
    # ── [v11.3 新增] action_smoothness_penalty (Mysore 2021 CAPS) ────────────
    r_act_smooth = 0.0
    if rl_action is not None and rstate.prev_rl_action is not None:
        diff = np.asarray(rl_action, np.float32) - np.asarray(rstate.prev_rl_action, np.float32)
        smooth_sq = float(np.mean(np.square(diff)))
        k_smooth = float(rcfg.get("action_smoothness_coef", 0.02))
        r_act_smooth = -k_smooth * smooth_sq
        reward += r_act_smooth
    if rl_action is not None:
        rstate.prev_rl_action = np.asarray(rl_action, np.float32).copy()
    ...
    # ── [v12 新增] 钢筋对准 bonus (rebar_align) ──────────────────────────────
    # 用户要求: descent 段与地面钢筋对准给奖励 (这是 descent 段精度的关键)
    # 设计: 当 xy_dist < rebar_align_radius 时每步小奖励, 鼓励 hold 在对准状态.
    # 这比 precision_bonus (高斯) 更密集, 但量级更小, 避免 dead critic.
    r_rebar = 0.0
    rebar_r = float(rcfg.get("rebar_align_radius", 0.010))
    rebar_close_r = float(rcfg.get("rebar_close_radius", 0.005))
    if dtf < rebar_r:
        # 线性奖励: 越近越大
        rebar_b = float(rcfg.get("rebar_align_bonus", 0.02))
        r_rebar = rebar_b * (1.0 - dtf / rebar_r)
        # 5mm 内双倍 bonus (鼓励超精度)
        if dtf < rebar_close_r:
            r_rebar *= 2.0
        reward += r_rebar
    if tracker: tracker.add("rebar_align_bonus", r_rebar)
    ...
    if rstate.insertion_hold_counter >= hold_steps:
        r_bonus = float(rcfg["success_bonus"])
        reward += r_bonus
        precision_coef  = float(rcfg.get("precision_bonus_coef", 10.0))
        sigma_prec      = float(rcfg.get("precision_bonus_sigma", 0.005))
        r_prec = precision_coef * float(np.exp(-0.5 * (dtf / sigma_prec) ** 2))
        reward += r_prec
        if tracker:
