...
class WelfordNormalizer:
    ...
    def normalize(self, x):
        ...
        # 至少有 2 个样本才计算方差，否则设为 1
        var = self.S / (self.n - 1) if self.n > 1 else np.ones_like(self.mean)
        norm_x = (x - self.mean) / np.sqrt(var + 1e-8)
...
class WBAgent:
    ...
    def __init__(self, log_dir, state_dim, action_dim, config=None, base_controller_func=None):
        ...
        self.target_q_abs_max = float(agent_cfg.get("target_q_abs_max", 300.0))
        self.critic_grad_clip = float(agent_cfg.get("critic_grad_clip", 1.0))
        self.actor_grad_clip  = float(agent_cfg.get("actor_grad_clip", 1.0))
        self.delayed_target_update = bool(agent_cfg.get("delayed_target_update", True))
        self.scale_target_noise_by_action = bool(
            agent_cfg.get("scale_target_noise_by_action", True)
        )
        self.bc_alpha      = float(agent_cfg.get("bc_alpha", 2.5))
        self.bc_coef       = float(agent_cfg.get("bc_coef", 1.0))
        self.bc_weight_min = float(agent_cfg.get("bc_weight_min", 0.05))
        self.bc_weight_max = float(agent_cfg.get("bc_weight_max", 10.0))

        reward_clip = agent_cfg.get("reward_clip", None)
        ...
        # +++ 【新增】状态归一化器 +++
        self.state_norm = WelfordNormalizer(shape=(state_dim,))
    ...
    def act(self, state):
        ...
        并在 Actor 接管时加入连续空间探索噪声。
        """
        with torch.no_grad():
            # 1. 专家控制器：使用真实物理 state
            base_action = (self.base_controller_func(state)
                           if self.base_controller_func
                           else np.zeros(self.action_dim))

            # 2. 神经网络 Actor：使用归一化后的 state
            norm_state = self.state_norm.normalize(state)
            s_tensor   = np_to_tensor(norm_state.reshape(1, -1))
            actor_action = self.actor(s_tensor).cpu().data.numpy().flatten()

            # +++ 【核心修复】：为 Actor 注入高斯探索噪声 +++
            # 设定探索噪声标准差（通常为动作上限的 10%）
            expl_noise_std = 0.01 * np.array(self.max_action, dtype=np.float32) 
            noise = np.random.normal(0, expl_noise_std, size=self.action_dim)
            
            # 叠加噪声并裁剪，防止突破物理极限
            noisy_actor_action = np.clip(actor_action + noise, -np.array(self.max_action, dtype=np.float32), np.array(self.max_action, dtype=np.float32))

            # Epsilon 贪心切换
            if random.random() < self.epsilon:
                return base_action, False, base_action
            else:
                return noisy_actor_action, True, base_action # 返回带噪声的动作
    ...
    def train(self, iterations):
        ...
        for _ in range(iterations):
            ...
            # +++ 【修改】将采样的真实状态批量归一化后，再转换为 Tensor +++
            norm_state = self.state_norm.normalize(state)
            norm_next_state = self.state_norm.normalize(next_state)

            device           = self._device()
            si               = np_to_tensor(norm_state).float()         # 使用 norm_state
            s_i              = np_to_tensor(norm_next_state).float()    # 使用 norm_next_state
            ...
            ri               = np_to_tensor(reward).float().view(-1, 1) # 记得保留上一次修复的 view!
            di               = np_to_tensor(done).float().view(-1, 1)   # 记得保留上一次修复的 view!

            if self.reward_clip is not None:
                ri = torch.clamp(ri, self.reward_clip[0], self.reward_clip[1])
            ...
            # ================================================================
            with torch.no_grad():
                if self.scale_target_noise_by_action:
                    noise_std = self.policy_noise * self.target_actor.max_action
                ...
                # ------------------------------------------------------------
                if self.base_boot:
                    next_base_act_t = torch.clamp(
                        next_base_act_t, -self.target_actor.max_action, self.target_actor.max_action
                    )
                ...
                '''if self.target_q_abs_max > 0:
                    yi = yi.clamp(-self.target_q_abs_max, self.target_q_abs_max)'''
            ...
            # +++ 【新增】记录 Q 值均值 +++
            total_q_pred += current_Q1.mean().item()
            total_q_target += yi.mean().item()

            if self.critic_loss_type == "mse":
                Lc = F.mse_loss(current_Q1, yi) + F.mse_loss(current_Q2, yi)
            else:
                Lc = F.smooth_l1_loss(current_Q1, yi) + F.smooth_l1_loss(current_Q2, yi)
            ...
            Lc.backward()
            torch.nn.utils.clip_grad_norm_(self.critic.parameters(), max_norm=self.critic_grad_clip)
            ...
            # ================================================================
            if not self.delayed_target_update:
            ...
            if self.total_it % self.policy_freq == 0:
                ...
                if self.behavior_clone:
                    ...
                    q_abs_mean = q_a.abs().mean().detach()
                    lmbda = 2.5 / (q_abs_mean + 1e-5) 
                    lmbda = torch.clamp(lmbda, max=100.0) # 加上绝对上限，防止初期失控
                    La = Lbc * lmbda - q_a.mean()

                    self._last_bc_loss = Lbc.item()'''
                    # --------------------------------------------------------
                    # 【真正安全的】Behavior Cloning (TD3+BC 官方逻辑)
                    # --------------------------------------------------------
                    a   = self.actor(si)
                    q_a = self.critic.Q1(si, a)

                    # 1. 计算纯粹的 MSE 克隆误差（绝对不能用 xi 掩码去关掉它！）
                    Lbc = ((a - base_action_t) ** 2).mean()

                    # 2. 计算动态缩放因子 lambda，并加入 1e-5 防除零保护
                    # 注意：detach() 非常关键，不能让梯度流向这里
                    q_abs_mean = q_a.abs().mean().detach()
                    lmbda = 0.5 / (q_abs_mean + 1e-5)   # zxy：0.5可调
                    
                    # 加上绝对上限保护，防止初始阶段 Q=0 时 lambda 飙升
                    lmbda = torch.clamp(lmbda, max=100.0)

                    # 3. 组合 Actor Loss
                    # 【极其关键的数学位置】：lmbda 必须乘在 q_a 上，而不是 Lbc 上！
                    # 这确保了 RL 的梯度永远不会淹没 BC 的监督信号。
                    La = Lbc - lmbda * q_a.mean()
        ...
        # +++ 【修改】返回 5 个变量 +++
        return (total_Lc / iterations, 
                self._last_actor_loss, 
                self._last_bc_loss,
                total_q_pred / iterations,    # 新增：预测Q均值
                total_q_target / iterations)  # 新增：目标Q均值
